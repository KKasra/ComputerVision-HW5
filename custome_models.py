...
def get_Q2_model():
	...
	model_output = keras.layers.Flatten()(model_output)
	model_output = keras.layers.LayerNormalization()(model_output)
	
	# model_output = keras.layers.Dropout(.3)(model_output)

	model_output = keras.layers.Dense(4096, activation='relu',kernel_regularizer = L2(.001), bias_regularizer = L2(.001))(model_output)
	# model_output = keras.layers.BatchNormalization()(model_output)
g
	model_output = keras.layers.LayerNormalization()(model_output)
...
def get_Q4_model():
	...
	output = keras.layers.Dense(15, kernel_regularizer=keras.regularizers.L2(.01),
									bias_regularizer=keras.regularizers.L2(.1))(output)
